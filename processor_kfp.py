...
class KfpPipelineProcessor(PipelineProcessor):
    ...
    def process(self, pipeline):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # use client auth session cookie instead of checking credential
            self.log_pipeline_info(pipeline_name, 'get credentials from cookie ')

            with open('/tmp/cookie','rt') as f:
                cookie = f.readline()
                session_cookie = f'authservice_session={cookie}'

            self.log_pipeline_info(pipeline_name, f'create a kfp session with {session_cookie} ')
        
            tags = runtime_configuration.metadata.get('tags')
            cluster_api_endpoint = [x.split('=')[1] for x in tags if x[:4] == 'api='][0]

            if cluster_api_endpoint :
                self.log_pipeline_info(pipeline_name, f'found a cluster api endpoint : {cluster_api_endpoint} ')
                client = kfp.Client(host=cluster_api_endpoint, cookies=session_cookie)
            else :
